...
NEW_SC = '''    STARTUP_CONFIG_XML = "/config/startup-config.xml"
    ...
    def _enable_jumbo_and_reboot(self, api_key):
        ...
        # PAN-OS 12 answers "Command succeeded" to <request><restart><system/> over the API but does
        # NOT reboot (verified: uptime kept climbing, jumbo stayed off, config commit failed again).
        # Use the one lever the guest cannot ignore: a QEMU system_reset on the monitor. It is a
        # clean power-cycle; PAN-OS boots normally and comes up in jumbo mode.
        self.logger.info("Phase 1: QEMU system_reset (API restart is a no-op on PAN-OS 12)")
        try:
            self.qm.write(b"system_reset\\r\\n")
        except Exception as exc:
            self.logger.warning("qemu monitor system_reset failed (%s); trying API restart", exc)
            self._api_op(api_key, "<request><restart><system></system></request></restart>", timeout=30)
        # detect the REAL down-then-up: the API must first FAIL (box rebooting), then succeed
        self.logger.info("waiting for the box to go down ...")
        went_down = False
