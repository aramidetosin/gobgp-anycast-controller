#!/bin/sh
# gobgp-1: GoBGP anycast controller (aggr peers 10.201.20.1/10.201.20.5, AS65400).
if ! ip addr show eth1 2>/dev/null | grep -q '10.201.20.2'; then
  ip link set eth1 up; ip link set eth2 up
  ip addr add 10.201.20.2/30 dev eth1; ip addr add 10.201.20.6/30 dev eth2
fi
...
ip route replace 192.168.202.0/24 nexthop via 10.201.20.1 dev eth1 nexthop via 10.201.20.5 dev eth2 2>/dev/null \
  || ip route replace 192.168.202.0/24 via 10.201.20.1 dev eth1
...
cp -f /bootstrap/gobgp/gobgp-1/gobgpd.conf /bootstrap/gobgp/gobgp-1/brain.json /etc/gobgp/ 2>/dev/null
...
echo "gobgp-1: eth1 10.201.20.2/30 eth2 10.201.20.6/30; gobgpd+brain up (aggr 10.201.20.1,10.201.20.5)"
