#!/bin/sh
# dc2-k8s-worker-3: first-boot L2/L3 bootstrap = the real host's netplan 01-fabric.yaml, as a script.
...
ip link set bond0 address 50:00:00:22:00:01
...
ip addr add 10.168.10.23/24 dev bond0
ip route replace default via 10.168.10.1 dev bond0 metric 50
printf 'nameserver 10.168.30.10\nsearch ecloud.lab\n' > /etc/resolv.conf
echo "dc2-k8s-worker-3: bond0 10.168.10.23/24 via 10.168.10.1 dns 10.168.30.10"
