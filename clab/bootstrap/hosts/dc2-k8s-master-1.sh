#!/bin/sh
# dc2-k8s-master-1: first-boot L2/L3 bootstrap = the real host's netplan 01-fabric.yaml, as a script.
...
ip link set bond0 address 50:00:00:1f:00:01
...
ip addr add 10.168.10.11/24 dev bond0
ip route replace default via 10.168.10.1 dev bond0 metric 50
printf 'nameserver 10.168.30.10\nsearch ecloud.lab\n' > /etc/resolv.conf
echo "dc2-k8s-master-1: bond0 10.168.10.11/24 via 10.168.10.1 dns 10.168.30.10"
